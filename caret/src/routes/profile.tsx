...
export const Profile = ({ user, setIsLoggedIn}) => {
  ...
  <p className = "text-sm  text-gray-400">{followers.length} followers</p>
    ...
    {followingNotif && (
        <div className="relative bg-blue-100 text-blue-500 p-3 rounded-md shadow-md mb-4">
          <p>{followingNotif}</p>
          <button
            className="absolute top-1 right-1 text-blue-700 font-bold"
            onClick={() => setfollowingNotif("")}
          >
            X
          </button>
        </div>
    ...
    {outputError && (
      <div className="relative bg-red-100 text-red-500 p-3 rounded-md shadow-md mb-4">
        <p>{outputError}</p>
        <button
